...
bool j1Render::PostUpdate()
{
	...
	if (App->scene->currentLevel != MAIN_MENU) {
		...
		if (App->scene->Player->player_position.x <= -(camera.x - camera.w / 4) && camera.x < 0)
			...
			camera.x -= App->scene->Player->player_velocity.x;
...
bool j1Render::DrawQuad(const SDL_Rect& rect, Uint8 r, Uint8 g, Uint8 b, Uint8 a, bool filled, bool use_camera, float scale_) const
{
	bool ret = true;
	float scale = scale_;
